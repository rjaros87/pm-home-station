...
class PNLineChart: UIView{
    private var oldXLabels = [UILabel]()
    private var oldYLabels = [UILabel]()
    ...
    public var chartData: NSArray = []{
        didSet{
            ...
            for f in stride(from: floor(yMin), to: ceil(yMax), by: 5) {
                yLabelsArray.append(f)
            }
